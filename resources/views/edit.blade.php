...
<body>
    <div class="container">
        <h2>แก้ไขข้อมูล</h2>
        <form action="{{ route('market.update', $markketadas->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="formGroupExampleInput" name="nameitem"
                    value="{{ $markketadas->nameitem }}" disabled>
            </div>
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="formGroupExampleInput2" name="name"
                    value="{{ $markketadas->name }}" >
            </div>
            <div>
                <p>สถานะสินค้า -> {{ $markketadas->status }}</p>
            ...
            <div class="pt-2">
                <select class="form-select" aria-label="Default select example" name="type">
                    <option selected>{{ $markketadas->type }}</option>
